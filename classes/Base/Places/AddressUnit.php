 ...
 * Instead, change the Places_AddressUnit.php file.
 *
 * @module Places
 */
/**
 * Base class representing 'AddressUnit' rows in the 'Places' database
 * @class Base_Places_AddressUnit
 * @extends Db_Row
 *
 ...
 */
abstract class Base_Places_AddressUnit extends Db_Row
{
	/**
	 * @property $id
	 * @type mixed
	...
	{
		if (class_exists('Q_Config') and Q_Config::get('Db', 'connections', 'Places', 'indexes', 'AddressUnit', false)) {
			return new Db_Expression(($with_db_name ? '{{dbname}}.' : '').'{{prefix}}'.'address_unit');
		} else {
			$conn = Db::getConnection('Places');
  			$prefix = empty($conn['prefix']) ? '' : $conn['prefix'];
  			$table_name = $prefix . 'address_unit';
  			if (!$with_db_name)
  				return $table_name;
  ...
  array (
	...
	static function insertManyAndExecute($rows = array(), $options = array())
	{
		self::db()->insertManyAndExecute(
			self::table(), $rows,
			array_merge($options, array('className' => 'Places_AddressUnit'))
		);
	}
...
return array (
  ...
  3 => NULL,
	...
	function beforeSet_unit($value)
	{
		...
		if (!is_string($value) and !is_numeric($value))
			throw new Exception('Must pass a string to '.$this->getTable().".unit");
		if (mb_strlen($value) > 31)
			throw new Exception('Exceedingly long value being assigned to '.$this->getTable().".unit");
		return array('unit', $value);			
	}
...
return array (
  ...
  3 => NULL,
	 ...
	 */
	function beforeSet_floor($value)
	{
		if (!isset($value)) {
			return array('floor', $value);
		}
		if ($value instanceof Db_Expression
               or $value instanceof Db_Range) {
			return array('floor', $value);
		...
		if (!is_numeric($value) or floor($value) != $value)
			throw new Exception('Non-integer value being assigned to '.$this->getTable().".floor");
		$value = intval($value);
		if ($value < -2147483648 or $value > 2147483647) {
			$json = json_encode($value);
			throw new Exception("Out-of-range value $json being assigned to ".$this->getTable().".floor");
		}
		return array('floor', $value);			
	}

	/**
	 * @method maxSize_floor
	 * Returns the maximum integer that can be assigned to the floor field
	 * @return {integer}
	 */
	function maxSize_floor()
	{
...
return array (
  ...
  3 => NULL,
	 ...
	 */
	function beforeSet_source($value)
	{
		if ($value instanceof Db_Expression
               or $value instanceof Db_Range) {
			return array('source', $value);
		}
		if (!in_array($value, array('loqate','assessment_roll','generated','user_correction')))
			throw new Exception("Out-of-range value '$value' being assigned to ".$this->getTable().".source");
		return array('source', $value);			
	}
...
return array (
  ...
  array (
    0 => 'enum',
    1 => '\'loqate\',\'assessment_roll\',\'generated\',\'user_correction\'',
    2 => '',
    3 => false,
  ),
  1 => false,
  ...
  3 => 'generated',
	 ...
	 * @throws {Exception} An exception is thrown if $value is not string or is exceedingly long
	 */
	function beforeSet_fullData($value)
	{
		if (!isset($value)) {
			return array('fullData', $value);
		}
		if ($value instanceof Db_Expression
               or $value instanceof Db_Range) {
			return array('fullData', $value);
		...
		if (!is_string($value) and !is_numeric($value))
			throw new Exception('Must pass a string to '.$this->getTable().".fullData");
		if (mb_strlen($value) > 65535)
			throw new Exception('Exceedingly long value being assigned to '.$this->getTable().".fullData");
		return array('fullData', $value);			
	}
	 ...
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
	static function column_fullData()
	{
...
return array (
  ...
  3 => NULL,
	 ...
	 * @throws {Exception} An exception is thrown if $value is not string or is exceedingly long
	 */
	function beforeSet_bbl($value)
	{
		if (!isset($value)) {
			return array('bbl', $value);
		}
		if ($value instanceof Db_Expression
               or $value instanceof Db_Range) {
			return array('bbl', $value);
		...
		if (!is_string($value) and !is_numeric($value))
			throw new Exception('Must pass a string to '.$this->getTable().".bbl");
		if (mb_strlen($value) > 15)
			throw new Exception('Exceedingly long value being assigned to '.$this->getTable().".bbl");
		return array('bbl', $value);			
	}
...
return array (
  ...
  3 => 'CURRENT_TIMESTAMP',
	...
	function beforeSave($value)
	{
		...
		if (!isset($this->fields["unit"]) and !isset($value["unit"])) {
	...
	{
		$field_names = array('id', 'address', 'unit', 'floor', 'source', 'fullData', 'bbl', 'insertedTime');
		$result = $field_names;
